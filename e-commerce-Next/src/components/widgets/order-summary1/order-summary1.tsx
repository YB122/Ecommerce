import { CheckCircle, CreditCard, Download, MapPin, Package, Printer, Truck } from "lucide-react";
...
import type { OrderSummary1Props, OrderSummaryData } from "./types";
    ...
    <section className={cn("py-16 md:py-24", className)}>
      <div className="container max-w-4xl">
                        ...
                        {item.details && (
                          <p className="mt-0.5 text-sm text-muted-foreground">
                            {item.details.map((d, i) => (
                              <span key={d.label}>{d.value}{i < item.details!.length - 1 && " \u00b7 "}</span>
                            ))}
                          </p>
                        )}
          ...
          <div className="space-y-6">
              ...
              <CardContent>
                {order.paymentMethod.type === "card" && (
                  <div className="flex items-center gap-3">
                    <Image src="https://deifkwefumgah.cloudfront.net/shadcnblocks/block/logos/visa-icon.svg" alt="Visa" width={32} height={32} />
                    <div>
                      <p className="text-sm font-medium">{order.paymentMethod.cardBrand} ending in {order.paymentMethod.lastFour}</p>
                ...
                {order.paymentMethod.type === "paypal" && (
                  <div className="flex items-center gap-3">
                    <div className="flex size-10 items-center justify-center rounded-md bg-muted">
                      <Image src="https://deifkwefumgah.cloudfront.net/shadcnblocks/block/logos/paypal-icon.svg" alt="PayPal" width={20} height={20} />
                    </div>
                    <div>
                      <p className="text-sm font-medium">PayPal</p>
                      <p className="text-sm text-muted-foreground">{order.paymentMethod.email}</p>
                    </div>
                  </div>
                )}
              </CardContent>
            ...
            <Card className="shadow-none">
              <CardContent className="space-y-3 p-4">
                <Button className="w-full" variant="default"><Package className="mr-2 size-4" />Track Order</Button>
                <Button className="w-full" variant="outline"><Download className="mr-2 size-4" />Download Receipt</Button>
                <Button className="w-full" variant="ghost"><Printer className="mr-2 size-4" />Print Order</Button>
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="mt-10 text-center">
          <p className="mb-4 text-muted-foreground">Have a question about your order?</p>
          <div className="flex flex-wrap justify-center gap-3">
            <Button variant="outline" render={<a href="#" />} nativeButton={false}>Contact Support</Button>
            <Button render={<a href="#" />} nativeButton={false}>Continue Shopping</Button>
          </div>
        </div>
      </div>
    </section>
