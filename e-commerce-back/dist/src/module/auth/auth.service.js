...
export const googleSignupHandler = (req, res) => {
    const user = req.user;
    if (!user) {
...
export const verifyEmail = async (req, res) => {
    try {
        ...
        const decoded = jwt.verify(token, env.verifySignature || "my");
...
export const forgotPasswordHandler = async (req, res) => {
    try {
        ...
        if (redis)
            await redis.set(`reset:${resetToken}`, String(user._id), { EX: 900 });
...
export const resetPasswordHandler = async (req, res) => {
    try {
        ...
        if (redisDel)
            await redisDel.del(`reset:${token}`);
...
export const refreshAccessToken = async (req, res) => {
    ...
    for (const role of validRoles) {
        ...
        if (!sig)
            continue;
        try {
            decoded = jwt.verify(token, sig + "_refresh");
            break;
        }
        catch { }
    }
