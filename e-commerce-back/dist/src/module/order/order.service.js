    ...
    try {
        ...
        for (const item of items) {
            ...
            if (variants[variantIdx].stock < item.quantity) {
                throw new Error(`Insufficient stock for ${product.en_name} (${item.color}/${item.size}). Available: ${variants[variantIdx].stock}, requested: ${item.quantity}`);
            }
        ...
        if (paymentMethod === "card") {
            const session = await createCheckoutSession(String(order[0]._id), grandTotal, orderItems.map(({ productId: _p, ...rest }) => rest));
            if (!session) {
