export const buildQuery = ({ reqQuery, priceField = "price", allowedSortFields = ["price", "name", "en_name", "createdAt"], defaultSort = ["createdAt", "DESC"], extraWhere = {}, searchFields = ["en_name", "ar_name", "fr_name"], }) => {
    ...
    if (reqQuery.minPrice !== undefined || reqQuery.maxPrice !== undefined) {
        const minPrice = reqQuery.minPrice !== undefined ? Number(reqQuery.minPrice) : undefined;
        const maxPrice = reqQuery.maxPrice !== undefined ? Number(reqQuery.maxPrice) : undefined;
    ...
    let order = { [defaultSort[0]]: defaultSort[1] === "DESC" ? -1 : 1 };
    if (sort) {
        const parts = sort.split("_");
        const field = parts.slice(0, -1).join("_");
        const dir = parts[parts.length - 1];
