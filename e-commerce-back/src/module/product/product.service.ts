...
export const createProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (colorImages) {
      ...
      for (const ci of arr) {
        ...
        if (ci.imageURL.startsWith("data:image/")) {
          ...
          } catch { colorImagesObj[ci.color] = ci.imageURL; }
        } else {
    ...
    const product = await productModel.create({
      ...
      variants: variants ? JSON.stringify((typeof variants === "string" ? JSON.parse(variants) : variants).map((v: any) => ({ color: v.color, size: v.size, stock: v.stock }))) : null,
...
export const updateProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (en_description !== undefined) product.en_description = en_description;
    if (ar_description !== undefined) product.ar_description = ar_description || null;
    if (fr_description !== undefined) product.fr_description = fr_description || null;
    if (price !== undefined) product.price = Number(price);
    if (discount !== undefined) product.discount = Number(discount);
    if (variants !== undefined) product.variants = variants ? JSON.stringify((typeof variants === "string" ? JSON.parse(variants) : variants).map((v: any) => ({ color: v.color, size: v.size, stock: v.stock }))) : null;
    if (colorImages !== undefined) {
      ...
      for (const ci of arr) {
        if (!ci.imageURL) continue;
        if (ci.imageURL.startsWith("data:image/")) {
          try { const r = await uploadBase64Image(ci.imageURL, "products"); obj[ci.color] = r.url; }
          catch { obj[ci.color] = ci.imageURL; }
        } else {
    ...
    if (addedByEmail !== undefined) product.addedByEmail = addedByEmail;
    ...
    if (files && files.length > 0) {
      try {
        if (product.imageURLs) {
          const oldUrls: string[] = JSON.parse(product.imageURLs);
